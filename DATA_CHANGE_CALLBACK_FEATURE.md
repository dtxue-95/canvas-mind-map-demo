...
## 2024-07-09 优先级二级菜单标签化与类型安全修复

- 新增：右键节点菜单"修改优先级"二级菜单，优先级选项以彩色圆角标签形式展示，风格与节点标签完全一致。
- 技术实现：
  1. ContextMenuAction.label 类型支持 ReactNode，允许自定义标签组件。
  2. 新增 PriorityLabel 组件，复用节点优先级标签样式。
  3. 生成优先级菜单项时，label 字段用 <PriorityLabel ... />，颜色、圆角、边框、背景与节点一致。
  4. 主菜单 action 必须有 onClick，children 字段为优先级选项。
- 类型安全：所有菜单项类型推断无误，支持多级菜单和自定义内容。
- 体验提升：优先级菜单视觉风格统一，交互直观，数据变更实时生效。
- 兼容性：不依赖任何 UI 组件库，纯 React+CSS 实现，兼容所有主流浏览器。
